...
print('tape set', tape_set)
substrate_set = list(np.unique(list(df['substrate'])))
print('length of df: ', len(df), substrate_set)
#
#
#
# App layout
...
layout = html.Div([
    ...
    html.Div(children=[
        ...
        html.Img(src=get_asset_url('scheme.png'), style={'display': 'inline-block', 'height': '300px'}),
        ]),
    html.Br(),
    html.Label([' Select a tape: '
                ],
               style={'font-size': 12, 'font-family': 'Calibri Light', "text-align": "center"}),
    html.Br(),
    ...
    dcc.Graph(id='peel-graph'),# style={'width': '100vh', 'height': '90vh'}),
...
def set_substrate_options(chosen_tape):
    ...
    print('substrates_of_tape: ', substrates_of_tape, values_selected)
    return substrates_of_tape, values_selected[0]
...
def build_graph(tape_set, substrate_set):
    ...
    indexes = df_.index
    print('name', name, 'substrate', substrate, indexes)
    ...
    for j in range(len(indexes)):
        ...
        df_raw_a = pd.read_pickle(DATA_PATH_raw.joinpath('raw_' + str(indexes[j])))
    ...
    plot_names = []
    string = []
    #print('all file names: ', df_['filename'])
    for u in range(0, len(df_)):
        ...
        if age[u] == 12:
            position = df_['filename'].iloc[u][0]
        else:
            position = ''
        #print('position bak or foran', position, age[u], df_['filename'].iloc[u][0])
        plot_names.append([age[u], Kini, Kini_std, Tpeel,
                           Tpeel_std, [round(z, 2) for z in df_['RMSE'].iloc[u]],
                           [round(z, 2) for z in df_['NRMSE'].iloc[u]], position])
        string.append('age:' + str(plot_names[u][0]) + ' ' + str(envir[u])+'<br />'\
                      #+ ' K-ini: ' + str(plot_names[u][1]) + '±' + str(plot_names[u][2])\
                      +' T-peel: ' + str(int(round(plot_names[u][3], 0))) + ' N'
                      #+ '±' + str(round(plot_names[u][4],1))\
                      #+ '\n' + ' NRMSE: ' + str(plot_names[u][6])
                      #+', cladding: '+str(position)+'___________________________|'
                        )
    ##### figure ###############
    fig = make_subplots(1, len(df_), shared_yaxes=False, subplot_titles=([str(i) for i in string]))
    ...
    fig.update_annotations(font=dict(family="Calibri Light", size=10))
    return fig#, string
