...
df = pd.read_pickle(DATA_PATH.joinpath('peel_all_code'))        # code or coded
...
def build_graph2(features, substrate_set, tape_set):
    ...
    df_decompose = pd.DataFrame(pca.components_,
                                ...
                                columns=features)
    decomp_1 = [round(k, 3) for k in list(df_decompose.iloc[0])]
    decomp_2 = [round(k, 3) for k in list(df_decompose.iloc[1])]
    decomp_3 = [round(k, 3) for k in list(df_decompose.iloc[2])]
    ...
    expl_var = [round(k, 3) for k in pca.explained_variance_ratio_]
    #
    # figure PCA
    #
    ##########
